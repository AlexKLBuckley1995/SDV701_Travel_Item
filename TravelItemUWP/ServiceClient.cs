...
namespace TravelItemUWP
{
    ...
    // Author: Alex Buckley
    // Date: 16/06/2017
    // Summary: This class is the interface to the server, so the UWP can call and receive from database queries in the TravelShopController 
    ...
    public static class ServiceClient
    {
        ...
        {
            return await InsertOrUpdateAsync(prOrder, "http://localhost:60064/api/TravelShop/PostOrder", "POST");
        }

        internal async static Task<string> UpdateOrderItemAsync(clsAllItem prItem)
        {
            return await InsertOrUpdateAsync(prItem, "http://localhost:60064/api/TravelShop/PutOrderItem", "PUT");
        }
    }
}
